...
API_KEY = "AIzaSyAk0CDdyUjhdqjpQ_NegHujeZiY5Ila-50"  # gemini API key
...
class CommandExplainer:
    ...
    def setup_ui(self):
        ...
        tk.Label(header_frame, text="Shell Command Explainer", font=("Helvetica", 14, "bold"),
                 fg="white", bg="#4285F4").pack(pady=10)
        ...
        tk.Label(input_frame, text="Enter command:", font=custom_font, bg="#f0f0f0").pack(side=tk.LEFT, padx=(0, 8))
        ...
        ttk.Button(input_frame, text="Explain", command=self.explain_command).pack(side=tk.LEFT, padx=(8, 0))

        output_frame = tk.Frame(main_frame, bg="#ffffff", bd=1, relief=tk.GROOVE)
        output_frame.pack(fill=tk.BOTH, expand=True)

        self.output_text = scrolledtext.ScrolledText(output_frame, wrap=tk.WORD,
                                                     font=custom_font, bg="white", fg="#333333",
                                                     padx=10, pady=10, spacing1=5, spacing2=2)
        ...
        tk.Label(self.root, textvariable=self.status_var, bd=1, relief=tk.SUNKEN, anchor=tk.W,
                 bg="#e0e0e0", fg="#333333", font=custom_font).pack(fill=tk.X)
    ...
    def fetch_explanation(self, command):
        try:
            ...
            # Save to history
            self.save_to_history(command, response.text)
    ...
    def display_result(self, explanation):
        ...
        for line in lines:
            if line.startswith(('PURPOSE', 'USAGE', 'OPTIONS', 'EXAMPLES')):
    ...
    # Added method to save command & explanation
    def save_to_history(self, command, explanation):
        try:
